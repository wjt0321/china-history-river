...
├── src/
│   ├── components/                # React 组件（每个组件 = .tsx + .css）
│   │   ├── MapView.tsx            # 地图主组件（MapLibre 初始化 + 疆域/都城图层管理）
│   │   ├── Timeline.tsx           # 河流时间轴（Canvas 2D 手绘，正弦曲线 + 渐变 + 脉冲高亮）
│   │   ├── DetailPanel.tsx        # 卷轴详情面板（5 Tab：概览/帝王/大事/文化/疆域）
│   │   ├── TopBar.tsx             # 顶部导航栏（朝代印章 + 下拉选择 + 详情开关）
...
│   │   └── dynasties-qing.ts
│   ├── stores/
│   │   └── appStore.ts            # Zustand store：selectedDynastyId / hoveredDynastyId / isDetailOpen
│   ├── styles/
...
# ESLint 检查（当前无自定义 eslint 配置文件，使用默认规则）
npm run lint
```

**重要**：项目没有配置单元测试或 E2E 测试。阶段 1 为视觉/内容导向的作品集案例，测试策略尚未建立。
...
- **MapView** 是唯一接触 MapLibre GL 的组件。所有地图图层（疆域填充、发光边线、都城标记）都在此组件内通过 `useRef` + `useEffect` 管理，避免 React 重渲染干扰 WebGL。
...
Zustand store（`src/stores/appStore.ts`）只存三个核心状态：
...
- **新增朝代数据**：在 `src/types/dynasty.ts` 确认接口 → 在 `src/data/` 新增或修改 TS 文件 → 在 `src/data/dynasties.ts` 导入导出 → 生成对应 GeoJSON 放入 `public/dynasties/` → 补充人物/场景图片映射到 `DetailPanel.tsx` 的 `FIGURE_IMAGES` / `SCENE_IMAGES`。
